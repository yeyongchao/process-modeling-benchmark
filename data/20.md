...
This system has three output variables $y(t)$ (reactor pressure $P_r$, level $V_L$, and temperature $T_r$), and the control (or manipulated) variables $u_l$ are the reactor feed rate $F_{in}$, the outlet-valve coefficient $\beta$ in the $F_{out}$ equation, and the cooling water flow rate $F_{CW}$. Thus, $\beta$ has a single role in this formulation and is not used as an agitator-speed symbol. For the purpose of this study, we assume that complete and accurate state information is available over all time. More details on the reactor model can be found in .

The NMPC controller needs to maintain conditions of reactor operation that correspond to equal production of products $G$ and $H$. This condition defines the supplied steady-state target $(z_{sp},y_{sp},u_{sp})$; in particular, $y_{sp}$ is the pressure, liquid-level, and temperature output at that equal-production operating point. However, at this operating point the reactor is open-loop unstable, and without closed-loop stabilizing control, the state profiles become unbounded. The dynamic model (9.61) can be represented as the following ODE system along with output equations: $\frac{dz(t)}{dt} = f(z(t),u), y(t) = g(z(t),u)$. The control problem is described over a prediction horizon with $N_T$ time periods of 180 seconds each. The objective function is to keep the reactor operating at the steady state point without violating the operating limits and constraints on the process variables.
...
$u_l = (F_{in,l},\beta_l,F_{CW,l})^T$: Vector of control (manipulated) variables at time period $l$ for $l=1,\dots,N_T$, where $\beta_l$ is the outlet-valve coefficient
...
$$\left(\sum_{i=A}^H N_{i,r} C_{p,i}\right)\frac{dT_r}{dt} = F_{in}(T_{in} - T_r)\sum_{i=A}^H y_{i,in} C_{p,vap,i} - Q_{CW}(T_r, T_{CW}, F_{CW}) - \sum_{j=1}^4 \Delta H_{R,j}(T_r) R_j$$
...
$$R_1 = \alpha_1 V_{Vr} \exp(\mu_1-\nu_1/T_r) P_{A,r}^{1.15} P_{C,r}^{0.370} P_{D,r}$$

$$R_2 = \alpha_2 V_{Vr} \exp(\mu_2-\nu_2/T_r) P_{A,r}^{1.15} P_{C,r}^{0.370} P_{E,r}$$

$$R_3 = \alpha_3 V_{Vr} \exp(\mu_3-\nu_3/T_r) P_{A,r} P_{E,r}$$

$$R_4 = \alpha_4 V_{Vr} \exp(\mu_4-\nu_4/T_r) P_{A,r} P_{D,r}$$

$$F_{out} = \beta_l \sqrt{P_r - P_s}$$
