...
Downstream processing is limited to 50,000 lb/stream hour of ethylene and 20,000 lb/stream hour of propylene.
...
* Feeds (cents/lb): Ethane 6.55, Propane 9.73, Gas oil 12.50, DNG 10.14.
* Products (cents/lb): Methane 5.38 (fuel value), Ethylene 17.75, Propylene 13.79, Butadiene 26.64, Gasoline 9.93, Fuel oil 4.50 (fuel value).
...
Combining product sales, feed costs, and purchased-fuel costs, the objective function (cents/h) is:
$$f = 2.84x_{1} - 0.22x_{2} - 3.33x_{3} + 1.09x_{4} + 9.39x_{5} + 9.51x_{6} - 5.38x_{7}$$
(The fixed heat loss contributes a constant part of the purchased-fuel cost and therefore does not change the optimizer, but the variable $x_7$ must retain its fuel-cost coefficient when it is kept in the model.)
...
2.  Ethylene processing limitation of 50,000 lb/h:
    $$0.5x_{1} + 0.35x_{2} + 0.20x_{3} + 0.25x_{4} + 0.5x_{5} + 0.35x_{6} \le 50,000$$
...
7.  Nonnegativity of all physical flow rates:
    $$x_i \ge 0,\qquad i=1,\ldots,7$$
